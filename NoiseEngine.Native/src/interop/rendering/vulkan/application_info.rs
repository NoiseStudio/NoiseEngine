...
use crate::{
    errors::invalid_operation::InvalidOperationError, interop::prelude::InteropString,
    rendering::vulkan::errors::universal::VulkanUniversalError,
};
        ...
        let application_name = match CString::new(&create_info.application_name) {
            ...
            Err(_) => {
                return Err(InvalidOperationError::with_str(
                    "Application name contains null character.",
                )
                .into())
            }
        };

        let engine_name = unsafe { CStr::from_bytes_with_nul_unchecked(b"NoiseEngine\0") };
